...
- [Operaciones con Máquinas Virtuales KVM](#operaciones-con-máquinas-virtuales-kvm)
  ...
  - [Pruebas y Validación](#pruebas-y-validación)
    ...
    - [Verificación de acceso SSH](#verificación-de-acceso-ssh)
      ...
      - [Acceso a Creacion\_virt\_install (máquina virtual creada con virt-install)](#acceso-a-creacion_virt_install-máquina-virtual-creada-con-virt-install)
      - [Acceso a clon\_virt\_manager (máquina virtual clonada con virt-manager)](#acceso-a-clon_virt_manager-máquina-virtual-clonada-con-virt-manager)
      - [Acceso a clon\_virt\_clone (máquina virtual clonada con virt-clone)](#acceso-a-clon_virt_clone-máquina-virtual-clonada-con-virt-clone)
      - [Acceso a clon\_copiando\_ficheros (máquina virtual creada manualmente)](#acceso-a-clon_copiando_ficheros-máquina-virtual-creada-manualmente)
...
```mermaid
flowchart TB
    A[Máquina Virtual Original<br>mvp1] --> B{Método de Clonación}

    B -->|GUI| C[virt-manager]
    C --> D[Clic derecho en MV]
    D --> E[Opción "Clonar"]
    E --> F[Configurar nombre y opciones]
    F --> G[clon_virt_manager]

    B -->|Línea de Comandos| H[virt-clone]
    H --> I["virt-clone --original mvp1 --name clon_virt_clone --file /path/to/new.qcow2"]
    I --> J[clon_virt_clone]
...
Esta tabla muestra una comparación visual de los diferentes métodos utilizados en la práctica:

```mermaid
%%{init: {'theme': 'forest'}}%%
quadrantChart
    title Métodos de Clonación/Creación de MV en KVM
    x-axis Complejidad
    y-axis Automatización
    quadrant-1 Eficiente
    quadrant-2 Complejo pero potente
    quadrant-3 Simple pero limitado
    quadrant-4 Óptimo
    "virt-install": [0.7, 0.6]
    "virt-clone": [0.3, 0.8]
    "virt-manager": [0.2, 0.7]
    "Copia Manual": [0.9, 0.2]
```
